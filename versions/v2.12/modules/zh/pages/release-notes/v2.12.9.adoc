...
:release-version: v2.12.9
:rn-component-version: v2.12
:previous-release-version: v2.12.8
...
*  A known issue occurs in air-gapped Harvester installations where the `ui-plugin-catalog` container `Compressed Endpoint` prevents installation. As a workaround, you can remove the `Compressed Endpoint` via `kubectl edit` using the example command `kubectl edit uiplugin harvester -n cattle-ui-plugin-system`. Once removed, the Harvester UI Plugin is immediately available. See https://github.com/rancher/rancher/issues/54659[#54659].
